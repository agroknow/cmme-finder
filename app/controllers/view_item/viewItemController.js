...
listing.controller("viewItemController", function($rootScope, $scope, $http, $location, $routeParams) {
	...
	$rootScope.getItem = function() {
		...
		$http.jsonp(item_query).success(function(data) {
			...
			if (thisJson.language.language_0 != undefined ) {
				$scope.item_language = thisJson.language.language_0;
			}
			else {
			...
			if (thisJson.creator) {
			if ( thisJson.creator.creator_0 ) {
				$scope.item_creator = thisJson.creator.creator_0;
			}
			else {
				$scope.item_creator = thisJson.creator;
			}
			}
			if (thisJson.provider.provider_0 != undefined ) {
				$scope.item_provider = thisJson.provider.provider_0;
			}
			else {
				$scope.item_provider = thisJson.provider;
			}
			...
			if (thisJson.contextUri.contextUri_0 != undefined ) {
				$scope.item_eu_url = thisJson.contextUri.contextUri_0;
			}
			else {
				$scope.item_eu_url = thisJson.contextUri;
			}
			$scope.item_content_type = thisJson.contentType;
			if ( $scope.item_provider =='RISM') {
				$scope.item_eu_id = $scope.item_eu_url;
			}
			else {
				$scope.item_eu_id = $scope.item_eu_url.match(/bsb[0-9]*/)[0];
			}
		})
